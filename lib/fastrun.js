...
import { ttyCols, stripAnsi, wrapAnsi, appEnv, runApp, parseCommandLine, loadState } from './core.js';
import { dkFileFor, readHeader, ensureExtracted, loadManifest } from './dk.js';
...
export async function fastRun(rest) {
  ...
  if (!name) { console.log(`${R}✗ Nom d'app manquant. Utilise: ${B}devkit run <nom>${X}\n`); return 1; }
  ...
  try {
    ...
    if (manifest.run && !noInstall && manifest.needDeps && !sharedNodeModules(dir)) {
    ...
    while (res.status !== 0 && repairs < MAX_REPAIRS) {
      ...
      if (noInstall && diag.needsInstall) {
        console.log(`${R}✗ Réparation impossible en mode ${B}--no-install${X}${R}. Relancez sans cette option.${X}\n`);
        return 1;
      }
